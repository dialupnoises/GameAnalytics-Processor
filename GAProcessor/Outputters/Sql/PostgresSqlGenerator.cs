...
namespace GAProcessor.Outputters.Sql
{
	public class PostgresSqlGenerator : ISqlGenerator
	{
		...
		public PostgresSqlGenerator(Config config)
		{
			_config = config;
		}
		...
		public string GenerateAlterStatement(string tableName, SqlColumn[] columns)
		{
			...
			foreach(var col in columns)
			{
				lines.Add($"ALTER TABLE {tableName} ALTER COLUMN {col.Name} TYPE {GetTypeName(col)};");
			}
		...
		private string GetTypeValue(SqlColumn col, string value)
		{
			switch(col.ColumnType)
			{
				...
				case SqlColumn.Type.Timestamp:
					...
					return "'" + dt.AddSeconds(double.Parse(value)).ToString("yyyy-MM-dd HH:mm:ss") + "+00'";
				default:
